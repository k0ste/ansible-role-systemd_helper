...
* [sysusers](https://www.freedesktop.org/software/systemd/man/sysusers.d.html#)
...
# systemd-logind is a system service that manages user logins. It is responsible
# for:
# * Keeping track of users and sessions, their processes and their idle state.
# This is implemented by allocating a systemd slice unit for each user below
# user.slice, and a scope unit below it for each concurrent session of a user.
# Also, a per-user service manager is started as system service instance of
# user@.service for each logged in user.
# * Generating and managing session IDs. If auditing is available and an audit
# session ID is already set for a session, then this ID is reused as the
# session ID. Otherwise, an independent session counter is used.
# * Handling of power/sleep hardware keys
# And other, consult with man systemd-logind

systemd_logind:
  - logind:
# Controls how logind shall handle the system power, reboot and sleep keys and
# the lid switch to trigger actions such as system power-off, reboot or suspend.
# Can be one of 'ignore', 'poweroff', 'reboot', 'halt', 'kexec', 'suspend',
# 'hibernate', 'hybrid-sleep', 'suspend-then-hibernate', 'lock', and
# 'factory-reset'. If 'ignore', systemd-logind will never handle these keys. If
# 'lock', all running sessions will be screen-locked; otherwise, the specified
...
# 'power-switch'udev tag will be watched for key/lid switch events.
      - handle_power_key: 'poweroff'
        handle_power_key_long_press: 'ignore'
        handle_reboot_key: 'reboot'
        handle_reboot_key_long_press: 'poweroff'
        handle_suspend_key: 'suspend'
        handle_suspend_key_long_press: 'hibernate'
        handle_hibernate_key: 'hibernate'
        handle_hibernate_key_long_press: 'ignore'
...
# time. This tool may be used to allocate system users and groups only, it is
# not useful for creating non-system (i.e. regular, "human") users and groups,
...
# Defines the route table name. The route table name must not be "default",
# 'main', or 'local', as these route table names are predefined with route
# table number 253, 254, and 255, respectively. The route table number must be
# an integer in the range 1...4294967295, except for predefined numbers
# 253, 254, and 255.
      route_table:
        - name: 'red'
          id: '100'
...
# Specifies the transmit queue length of the device in number of packets. When
...
# kernel's default will be used
    eth_lro: 'false'
# If set to 'true', receive VLAN CTAG hardware acceleration is enabled. When
# unset, the kernel's default will be used
    eth_rxvlan: 'false'
# If set to 'true', transmit VLAN CTAG hardware acceleration is enabled. When
# unset, the kernel's default will be used
    eth_txvlan: 'false'
# If set to 'true', receive filtering on VLAN CTAGs is enabled. When unset, the
# kernel's default will be used
    eth_rx_vlan_filter: 'false'
# If set to 'true', transmit VLAN STAG HW acceleration is enabled. When unset,
# the kernel's default will be used
    eth_tx_vlan_stag: 'false'
# If set to 'true', receive N-tuple filters and actions are enabled. When unset,
# the kernel's default will be used
    eth_rx_ntuple_filter: 'false'
...
# will be used
    eth_tx_ring: ''
# When set, enables receive flow control, also known as the ethernet receive
# PAUSE message (generate and send ethernet PAUSE frames). When unset, the
# kernel's default will be used
    eth_rx_flow: 'false'
# When set, enables transmit flow control, also known as the ethernet transmit
# PAUSE message (respond to received ethernet PAUSE frames). When unset, the
# kernel's default will be used
    eth_tx_flow: 'false'
# When set, auto negotiation enables the interface to exchange state
# advertisements with the connected peer so that the two devices can agree on
# the ethernet PAUSE configuration. When unset, the kernel's default will be
# used
    eth_flow_control_autoneg: 'false'
  - interface: 'po1'
    type: 'bond'
    mtu: '9000'
...
# is requested by router advertisement. By default, the token is autogenerated.
      ipv6_token: ''
# Takes a boolean. Controls IPv6 Router Advertisement (RA) reception support for
# the interface. If true, RAs are accepted; if false, RAs are ignored, independently
# of the local forwarding state. When RAs are accepted, they may trigger the start
# of the DHCPv6 client if the relevant flags are set in the RA data, or if no routers
# are found on the link.
